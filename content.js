...
;(function () {
    ...
    function findAllElementsInShadowDOM(root, selector) {
        ...
        for (const el of elementsWithShadow) {
            if (el.shadowRoot) {
                const found = findAllElementsInShadowDOM(
                    el.shadowRoot,
                    selector
                )
                results.push(...found)
            ...
            const checkInterval = setInterval(() => {
                ...
                try {
                    ...
                    if (!appRoot || !appRoot.shadowRoot) {
                        console.log(
                            `Attempt ${attempts}: app-root or shadow DOM not ready`
                        )
                    ...
                    console.error(
                        'Error in waitForAppRootShadowDOM check:',
                        error
                    )
                    if (attempts >= maxAttempts) {
                        clearInterval(checkInterval)
    ...
    // Function to extract text from web components with better cleaning
    function extractTextFromWebComponent(element) {
        if (!element) return ''
        ...
        try {
            ...
            if (element.shadowRoot) {
                text = getAllTextFromShadowDOM(element)
            } else {
            ...
            console.error('Error extracting text from web component:', error)
            return ''
        }
    }

    // Function to get all text content from shadow DOM recursively
    function getAllTextFromShadowDOM(element) {
        if (!element) return ''
        ...
        try {
            ...
            console.error('Error getting all text from shadow DOM:', error)
            return ''
        }
    }
    ...
    async function extractData() {
        try {
            ...
            if (!orderTable || !orderTable.shadowRoot) {
                throw new Error(
                    'order-table component or its shadow DOM not found'
                )
            }

            console.log(
                'Found order-table component in shadow DOM, extracting data...'
            )
            ...
            if (dataRows.length === 0) {
                throw new Error(
                    'No product data could be extracted from the order table'
                )
            }
            ...
            console.log(
                `Successfully extracted ${dataRows.length} rows of data`
            )
        ...
        try {
            ...
            tableRows.forEach((row, index) => {
                ...
                try {
                    ...
                    const payCell = row.querySelector(
                        'td:nth-child(2) order-table-pay'
                    )
    ...
    function extractRowData(
        ...
        try {
            ...
            if (goodsCell && goodsCell.shadowRoot) {
                const imgElement = findElementInShadowDOM(
            ...
            if (goodsCell && goodsCell.shadowRoot) {
                ...
                if (goodsInfo) {
                    ...
                    if (skuDiv) {
                        ...
                        const productNumber =
                            skuDiv.querySelector('.product-number')
                        if (productNumber) {
            ...
            if (payCell && payCell.shadowRoot) {
                ...
                if (payDiv) {
                    ...
                    if (!targetPriceDiv) {
                        targetPriceDiv =
                            payDiv.querySelector('div:nth-child(1)')
                    }
        ...
        try {
            ...
            if (orderInfoComponent && orderInfoComponent.shadowRoot) {
                ...
                for (const item of userContentItems) {
                    ...
                    if (
                        fieldName &&
                        fieldName.textContent.includes('会员登录名')
                    ) {
